...
当前项目版本为 `1.0.2`，唯一来源是根目录 `package.json`。代理包、Capacitor 包、Android 的 `versionName`、iOS 的 `MARKETING_VERSION` 和 Web 产物 manifest 都会与它校验一致：
...
发布标签统一使用 `v<版本号>` 格式；发布前可用 `node scripts/verify-version.mjs v1.0.2` 显式校验。推送匹配标签后会自动构建并发布 Android、iOS 和 Web 三类产物。已有历史标签不回写。
