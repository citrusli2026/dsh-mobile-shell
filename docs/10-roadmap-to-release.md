...
状态：**W1–W4 已完成（2026-09-04）**。W2 设备生命周期、W3 管理与部署易用性、W4 i18n/无障碍已交付，设计见 [ADR-0010](decisions/ADR-0010-device-registry-admin-i18n.md)，部署指南见 [12](12-reverse-proxy-and-networking.md)。验证证据：真实 dsh HTTP 矩阵 36/36、HTTPS/WSS 矩阵 36/36、设备生命周期矩阵（含损坏状态、重启持久化、到期 fail-closed）18/18、Playwright Chromium/WebKit/移动 Chrome/移动 Safari 24/24（扫码、退出、吊销、admin、双语切换）。下一站是第 7 节 Web 稳定版门禁的真实模型流与干净 checkout CI 复验。
...
| 设备会话 | ✅ 已完成（W2） | `DSH_STATE_FILE` 设备登记（0600、原子写、损坏 fail-closed）；`/session/check`、`/device/logout`、`/devices`、`/devices/revoke` 全部落地并有 HTTP/WS/浏览器证据 |
| Web 管理 | ✅ 已完成（W3） | `/admin` 管理台（设备列表、单独吊销、新配对码、SVG QR）；`healthz` 上游探测；启动日志含监听/TLS/候选地址；反代配置见 [12](12-reverse-proxy-and-networking.md) |
| Web 文案与可访问性 | ✅ 已完成（W4） | 启动页中英双语（浏览器协商 + 手动切换）；`data-testid`、`role=status`+live region、focus-visible、320px 窄屏 |
| Android/iOS | ⏸ 后移 | 构建脚本、正式签名、真机门禁问题已经记录，待 Web 稳定版门禁通过后恢复 |
...
## 5. W3：Web 管理与部署易用性（✅ 已完成，2026-09-04）
...
## 6. W4：国际化、无障碍与窄屏体验（✅ 已完成，2026-09-04）
