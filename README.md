...
```sh
tar -czf dist/dsh-mobile-shell-v1.0.2-web.tar.gz -C dist/web .
```
...
**Upstream authentication:** `dsh web` releases since 0.1.2-alpha.2 serve the UI's index document only to a per-launch token or a signed browser cookie. Pass the launch token as `DSH_UPSTREAM_TOKEN` and the proxy exchanges it once for that cookie and presents it on every forwarded request — devices still authenticate only to the proxy. Without it, an authenticated kernel answers forwarded index requests with its own 401.
...
The current project version is `1.0.2`, declared once in the root `package.json`. The proxy package, Capacitor package, Android `versionName`, iOS `MARKETING_VERSION`, and Web artifact manifest are checked against it:
...
Release tags use the `v<version>` form; validate one explicitly with `node scripts/verify-version.mjs v1.0.2` before publishing. Pushing a matching tag runs the Android, iOS, and Web builds and publishes all three assets. Existing historical tags are not rewritten.
